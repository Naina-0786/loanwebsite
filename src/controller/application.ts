...
export const grtAllPayment = asyncHandler(async (req, res, next) => {
    ...
    if (!payment) return next(new ErrorResponse("payment is not found", 201))
...
export const updateLoanApplication = asyncHandler(async (req: Request, res, next) => {
        ...
        if (file) {
            imageUploadPromises.push(
                handleImageUpload(
                    file,
                    fieldName,
                    existingApplication[fieldName as keyof typeof existingApplication],
                    applicationId
                ).then((imageData) => {
                    if (imageData) {
...
const imageFields: ImageFieldKeys[] = [
  "processingFee",
  "bankTransactionPaperFee",
  "insuranceFee",
  "cibilFee",
...
export const deleteLoanApplication = asyncHandler(
  async (req, res, next) => {
    ...
    return SuccessResponse(
      res,
      "Loan application and associated images deleted successfully",

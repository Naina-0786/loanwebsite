...
export const AdminCreate = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
...
export const updateAdmin = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    const id = Number(req.params.id);
    if (!id) return next(new ErrorResponse("ID is requiredd", 400));
    ...
    if (!updatedAdmin) return next(new ErrorResponse("Admin not found", 404));

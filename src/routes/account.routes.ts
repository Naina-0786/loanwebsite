...
AccountNumberRoute.put(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    if (!accountNumber || !bankName || !ifscCode) {
      return next(
        new ErrorResponse(
          "accountNumber, bankName, and ifscCode are required",
...
AccountNumberRoute.get(
  ...
  asyncHandler(async (req, res, next) => {
    try {
      ...
      if (!record) {
        return next(
          new ErrorResponse("No account number found", statusCode.Not_Found)
        );
      }

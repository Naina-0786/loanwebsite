...
PaymentFeeRoute.put(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    try {
      ...
      if (bankTransactionPaperFee) updateData.bankTransactionPaperFee = bankTransactionPaperFee;
      if (insuranceFee) updateData.insuranceFee = insuranceFee;
      if (cibilFee) updateData.cibilFee = cibilFee;
      if (tdsFee) updateData.tdsFee = tdsFee;
      ...
      return next(
        new ErrorResponse('Failed to update payment fee configuration', statusCode.Internal_Server_Error)
      );
    }
  })
);
...
PaymentFeeRoute.get(
  ...
  asyncHandler(async (req, res, next) => {
    try {
      ...
      if (!paymentFees) {
        return next(
          new ErrorResponse('Payment fee configuration not found', statusCode.Not_Found)
        );
      }
      return SuccessResponse(res, 'Payment fee configuration fetched successfully', paymentFees);
    } catch (error) {
      return next(
        new ErrorResponse('Failed to fetch payment fee configuration', statusCode.Internal_Server_Error)
      );
    }
  })
);
